...
{
    public partial class MathQ : Form
    {
        // A local integer variable is created with a scope of the MathQ form that holds the value of the number of questions
        private int numOfQVal;

        // A local integer variable is created with a scope of the MathQ form that holds the next question number
        private int nextQNo;

        // A local integer variable is created with a scope of the MathQ form that holds the number of questions answered correctly
        private int numOfQAnsCorr;

        // Creates an instance of the object MathQActivity
        private MathQActivity activity = new MathQActivity();
        ...
        // Method that closes the MathQ form and displays the main menu form to the user
        ...
        private void check_btn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection())
                {
                    ...
                    {
                        // Declares two new integer variable called floatAnswer and answer and initialises them as 0
                        float floatAnswer, answer = 0;

                        // Parses both the user's answer and the stored answer to float
                        if (float.TryParse(answer_textbox.Text, out floatAnswer) && float.TryParse(activity.getAnswer(), out answer))
                        {
                        ...
                        {
                            // Tells the user to enter a valid answer
                            MessageBox.Show("Please enter a number to 3 s.f. in the form '6.63E-34'");
                        }
                    }
                    else
                    {
